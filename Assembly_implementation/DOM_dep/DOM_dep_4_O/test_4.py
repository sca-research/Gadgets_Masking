...
def uart(ser_port, num_repeat):
    ...
    input_length = 25  # a: 1B, b: 1B, rnd: 23B (4B for masking a, 4B for masking b, 5B for z (refresh) 10B for Dom_indep_4 multiplication)

    # The length of the RX data in Byte (B)
    output_length = 18  # a: 1B, b: 1B, shares_a: 5B, shares_b: 5B, shares_a*b: 5B, output of gmul func in main.c: 1B
    ...
    for i in range(num_repeat):
        ...
        shares_ab = rx_[12:17]
        gmul = rx_[17]
        ab = shares_ab[0] ^ shares_ab[1] ^ shares_ab[2] ^ shares_ab[3] ^ shares_ab[4]
        a = shares_a[0] ^ shares_a[1] ^ shares_a[2] ^ shares_a[3] ^ shares_a[4]
        b = shares_b[0] ^ shares_b[1] ^ shares_b[2] ^ shares_b[3] ^ shares_b[4]
