...
/*The number of randomness for Dom_dep: Mask_ORD+1 + Mask_order*(Mask_order + 1)/2: 2+1+ 2*(2+1)/2=6
The number of randomness for masking the inputs: 2* Mask_order: 2*2=4
 Total rnd = 6 + 4 =10*/
//int rnd_n = 10;
...
int main( int argc, char* argv[]){
    ...
    while(true) {
        ...
        for (int i = 0; i < 6; i++) {
            rnd_dom_2[i] = (uint8_t) scale_uart_rd(SCALE_UART_MODE_BLOCKING);
        }

        Mask(in_a, rnd_a, shares_a);
        Mask(in_b, rnd_b, shares_b);
        Dom_dep_2(shares_a, shares_b, rnd_dom_2, shares_ab);
