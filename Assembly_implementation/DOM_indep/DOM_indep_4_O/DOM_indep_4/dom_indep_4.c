...
/*The number of randomness for Dom_indep: Mask_order*(Mask_order + 1)/2: 4*(5+1)/2=10
...
int main( int argc, char* argv[]){
    ...
    while(true) {
        ...
        for (int i = 0; i < 10; i++) {
            rnd_dom_4[i] = (uint8_t) scale_uart_rd(SCALE_UART_MODE_BLOCKING);
        }

        Mask(in_a, rnd_a, shares_a);
        Mask(in_b, rnd_b, shares_b);
        Dom_indep_4(shares_a, shares_b, rnd_dom_4, shares_ab);
