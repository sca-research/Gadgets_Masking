...
/*The number of randomness for ISW: Mask_order*(Mask_order + 1)/2: 2*(2+1)/2=3
...
int main( int argc, char* argv[]){
    ...
    while(true) {
        ...
        for (int i = 0; i < 3; i++) {
            rnd_isw[i] = (uint8_t) scale_uart_rd(SCALE_UART_MODE_BLOCKING);
        }

        Mask(in_a, rnd_a, shares_a);
        Mask(in_b, rnd_b, shares_b);
       Isw_2(shares_a, shares_b, rnd_isw, shares_ab);
