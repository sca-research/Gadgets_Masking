...
def uart(ser_port, num_repeat):
   ...
   # input_length = 5  # a: 1B, b: 1B, rnd: 3B (1B for sharesing a, 1B for sharesing b, 1B for ISW multiplication)
    ...
    output_length = 12  # a: 1B, b: 1B, shares_a: 2B, shares_b: 2B, shares_a*b: 2B, output of gmul func in main.c: 1B
    ...
    for i in range(num_repeat):
        ...
        input_data = [0x87, 0x10, 0x25, 0x10, 0xd2, 0xab, 0xf8, 0x41, 0x08]
        ...
        shares_ab = rx_[8:11]
        gmul = rx_[11]
        ab = shares_ab[0] ^ shares_ab[1] ^ shares_ab[2]
        a = shares_a[0] ^ shares_a[1] ^ shares_a[2]
        b = shares_b[0] ^ shares_b[1] ^ shares_b[2]
        #
        # if gmul != ab:
