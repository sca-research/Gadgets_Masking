...
/*The number of randomness for Dom_indep: Mask_order*(Mask_order + 1)/2: 1*(1+1)/2=1
The number of randomness for optimized refresh mask: Mask_order = 1
...
int main( int argc, char* argv[]){
    ...
    while(true) {
        ...
        for (int i = 0; i < 2; i++) {
            rnd_hpc[i] = (uint8_t) scale_uart_rd(SCALE_UART_MODE_BLOCKING);
        }

        Mask(in_a, rnd_a, shares_a);
        Mask(in_b, rnd_b, shares_b);
        HPC1_opt_1(shares_a, shares_b, rnd_hpc, shares_ab);
