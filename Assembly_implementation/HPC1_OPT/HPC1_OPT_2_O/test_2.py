...
def uart(ser_port, num_repeat):
    ...
    input_length = 11  # a: 1B, b: 1B, rnd: 9B (2B for sharesing a, 2B for sharesing b, 2B for OPT_refreshing, 3B for Dom_indp_2 multiplication)

    # The length of the RX data in Byte (B)
    output_length = 12 # a: 1B, b: 1B, shares_a: 3B, shares_b: 3B, shares_a*b: 3B, output of gmul func in main.c: 1B
    ...
    for i in range(num_repeat):
        ...
        shares_ab = rx_[8:11]
        gmul = rx_[11]
        ab = shares_ab[0] ^ shares_ab[1] ^ shares_ab[2]  # c = a * b

        if gmul != ab:
            print("ERROR: XOR(shares_ab)!= gmul(in_b, in_a)")
            break

        print('- out{}:  [{}]'.format(i, rx_.hex()))
