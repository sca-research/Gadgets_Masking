...
/*The number of randomness for Dom_indep: Mask_order*(Mask_order + 1)/2: 3*(3+1)/2=6
 * The number of randomness for opt refresh masking: 4
...
int main( int argc, char* argv[]){
    ...
    while(true) {
        ...
        for (int i = 0; i < 10; i++) {
            rnd_hpc_3[i] = (uint8_t) scale_uart_rd(SCALE_UART_MODE_BLOCKING);
        }

        Mask(in_a, rnd_a, shares_a);
        Mask(in_b, rnd_b, shares_b);
        HPC1_opt_3(shares_a, shares_b, rnd_hpc_3, shares_ab);
